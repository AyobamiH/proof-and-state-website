...
const POSTURE = [
  {
    ...
    value: "DoneState prepares reviewable repository changes and does not claim the final merge decision for itself.",
  },
  {
    key: "Inspection without mutation",
    value: "OpsTruth is designed to inspect software evidence without deploying, merging, publishing or restarting the system it checks.",
  },
  {
    key: "Credential control",
    value: "Operators remain responsible for repository access, secret storage and the credentials used in their own environments.",
  },
  {
    key: "Product maturity",
    value: "Security-relevant behaviour that is still planned, including AgentProof design goals, is not presented as already shipped.",
  },
  {
    key: "Public source",
    value: "Repositories and published releases can be inspected directly instead of relying only on security marketing copy.",
  },
  {
  ...
  return (
    <>
      ...
      <Section id="posture">
        ...
        />
        <div className="mt-10 max-w-3xl"><KeyValueRows rows={POSTURE} /></div>
      </Section>

      <Section id="scope">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,5fr)_minmax(0,6fr)] lg:gap-16">
          <SectionHeading id="scope" eyebrow="Operator responsibilities" title="What remains yours" />
          <div>
            <Prose>
              <p>You remain responsible for repository access, secret storage, network policy, dependency choices and the security of the systems the tools interact with.</p>
              <p>Reviewable automation reduces some operational risk; it does not remove the need to review access and changes in the context of your own environment.</p>
            </Prose>
            <Callout tone="risk" title="No compliance claims" className="mt-8">
              Proof &amp; State makes no SOC 2, ISO, HIPAA, PCI or GDPR compliance claim on this site. Nothing here should be treated as an audit result or certification.
            </Callout>
          </div>
        </div>
      </Section>
      ...
      <Section id="disclosure">
        ...
        <div className="mt-8 max-w-2xl space-y-4 text-[0.9375rem] leading-relaxed text-muted-foreground">
          <p>Email <a href={`mailto:${SECURITY_EMAIL}`} className="font-mono text-info underline underline-offset-4">{SECURITY_EMAIL}</a> with the affected repository or component, the issue and reproduction steps.</p>
          <p>A private security advisory on the relevant GitHub repository is also appropriate where available.</p>

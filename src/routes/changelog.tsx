    ...
    <>
      ...
      <Section id="entries">
        <ol className="space-y-10">
          {CHANGELOG.map((entry) => (
            <li key={`${entry.layer}-${entry.version}`} className="grid gap-4 lg:grid-cols-[minmax(0,3fr)_minmax(0,8fr)]">
              <div>
                <p className="font-mono text-[0.75rem] text-muted-foreground">{entry.date}</p>
                <p className="mt-1 font-mono text-[0.8125rem] text-foreground">{entry.layer} {entry.version}</p>
              </div>
              <div>
      ...
      <Section id="policy">
        ...
        <ul className="mt-8 max-w-3xl space-y-px overflow-hidden rounded-[10px] border border-hairline bg-hairline">
          {CHANGELOG_POLICY.map((rule) => (
            <li key={rule} className="bg-card px-5 py-4 text-[0.9375rem] leading-relaxed text-foreground">{rule}</li>
          ))}
        </ul>
      </Section>

      <Section id="source">
        <SectionHeading id="source" eyebrow="Source" title="Follow the repositories" lead="Repositories remain the authoritative record for implementation detail and older release history." />

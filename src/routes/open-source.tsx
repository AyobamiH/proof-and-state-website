  ...
  return (
    <>
      <PageHeader
        ...
        aside={
          <a href={GITHUB_URL} target="_blank" rel="noreferrer noopener" className="inline-flex items-center gap-1.5 rounded-[8px] border border-hairline bg-card px-4 py-2.5 font-mono text-[0.75rem] text-muted-foreground transition-colors hover:text-foreground">
      ...
      <Section id="repositories">
        <SectionHeading id="repositories" eyebrow="Repositories" title="Public source for each product" />
        <ul className="mt-10 grid gap-px overflow-hidden rounded-[10px] border border-hairline bg-hairline lg:grid-cols-3">
          {PRODUCTS.map((product) => (
            <li key={product.key} className="bg-card p-6">
              <div className="flex items-center justify-between gap-3"><p className="font-display text-xl font-medium text-foreground">{product.name}</p><StateChip tone={product.accent}>{product.stateBadge}</StateChip></div>
              <p className="mt-3 text-[0.875rem] leading-relaxed text-muted-foreground">{product.definition}</p>
              <a href={product.repo} target="_blank" rel="noreferrer noopener" className="mt-5 inline-flex items-center gap-1.5 font-mono text-[0.75rem] text-info underline underline-offset-4">
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,5fr)_minmax(0,6fr)] lg:gap-16">
          ...
          <div className="space-y-5">
            <Callout tone="verified" title="OpsTruth">
              {OPSTRUTH_STATUS} <a href={OPSTRUTH_MARKETPLACE_URL} target="_blank" rel="noreferrer noopener" className="underline underline-offset-4">Open the Marketplace listing.</a>
            </Callout>
      ...
      <Section id="project-record">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,5fr)_minmax(0,6fr)] lg:gap-16">
          <SectionHeading id="project-record" eyebrow="Project record" title="Portfolio decisions are documented separately" />
          <Prose>
            <p>The Proof &amp; State project record is maintained in a separate public repository so product and website source do not have to double as the public marketing narrative.</p>
            <p><a href={GOVERNANCE_REPO_URL} target="_blank" rel="noreferrer noopener">Open the Proof &amp; State project record on GitHub.</a></p>
            <p>When this site and a product repository disagree about implementation detail, prefer the current product repository and verified release record.</p>
          </Prose>
        </div>
      </Section>

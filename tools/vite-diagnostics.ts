    ...
    configureServer(server) {
      server.middlewares.use((request, response, next) => {
        response.once("finish", () => {
          ...
          const data = { service: "proof-and-state-website", kind: "server-response", status: response.statusCode };
          server.config.logger.error(`[proof-state:server-error] HTTP ${response.statusCode}; inspect the server error log.`);
